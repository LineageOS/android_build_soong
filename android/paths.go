...
func ApplySourceOverlays(ctx ModuleContext, directives string, allowedModules []string, srcFiles []Path) []Path {
	...
	for _, directive := range strings.Fields(directives) {
		// Split directives string into whitespace fields.
		...
		// Glob dir/globpattern to establish which overlay files exist.
		...
		// Add globbed paths to overlayMap.  First match wins.
		for _, path := range paths {
			// Skip directories
	...
	// Replace entries in srcFiles that exist in overlayMap.
	for i := range srcFiles {
		...
		if overlayFile, found := overlayMap[srcFileSuffix]; found {
			...
			delete(overlayMap, srcFileSuffix)
		}
	}
